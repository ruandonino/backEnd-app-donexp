...
Calcado.findByProduct = async function (idProduct,client_id,date,shop_id, tam, color,quant, result) {
  ...
  if(data_verify === undefined){
    ...
    try{
      ...
      var ret_insert_order = await dbConn.execute("INSERT INTO ORDER_ (CLIENT_ID,DATE_,TOTAL_VALUE,ID_SHOP) VALUES (:1,:2,:3,:4) returning ID into :return_id", {1:Number(client_id),2:date,3:Number(prod.rows[0][0]),4:Number(shop_id)},{ autoCommit: true });
    }catch{
      console.log("error: ", err);
    }
    finally{
    ...
    //console.log(newCalcado.calcado);
  try{    
    var ret_item_order = await dbConn.execute("INSERT INTO ITEM_ORDER (ID_ORDER,ID_PRODUTO,QUANT,ID_ITEM,CATEGORY) VALUES (:1,:2,:3,:4,:5)",{1:id_order,2:id_produto,3:quant,4:id_item,5:"CALCADO",return_id:{
      dir: oracledb.BIND_OUT,
      type: oracledb.NUMBER
    }},{ autoCommit: true });
  }
