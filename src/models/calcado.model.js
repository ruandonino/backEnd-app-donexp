...
Calcado.create = async function (newCalcado,result) {
    ...
    if(data_verify === undefined){
      ...
      try{
        var ret_produto = await dbConn.execute("INSERT INTO PRODUTO (NAME,PRICE,BRAND,ID_SHOP,TYPE) VALUES (:1,:2,:3,:4,:5) returning ID into :return_id", newCalcado.produto,{ autoCommit: true });
      }catch{
        console.log("error: ", err);
      }
      finally{
...
Calcado.findByIdShop = async function (id, result) {
  var dbConn = await checkConnection();
  try{
    var ret = await dbConn.execute("SELECT * FROM PRODUTO WHERE ID_SHOP = :id AND TYPE = 'CALCADO'", [id]);
    console.log(ret);
  }
  catch(err) {
    console.log("error: ", err);
